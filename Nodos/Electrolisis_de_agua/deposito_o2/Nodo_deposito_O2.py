...
async def nodo_deposito_O2():
    ...
    try:
        ...
        nodo_estado = await cliente.nodes.root.get_child(
            ["0:Objects", f"{ns_local}:Deposito_O2", f"{ns_local}:Estado"]
        )
        ...
        while True:
            ...
            nueva_cantidad_O2 = obtener_cantidad_O2(cantidad_anterior_O2, estado=estado_actual)
            if nueva_cantidad_O2 is not None:
                cantidad_actual_O2 = nueva_cantidad_O2
...
class ProtocoloHandler:
    ...
    def datachange_notification(self, node, val, data):
        print(f"Deposito O2 recibió estado: {val}")
